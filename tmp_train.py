...
def main() : 
    ...
    os.environ['WANDB_DIR'] = workspace + args.name + "/wandb/"
    os.environ['WANDB_CACHE_DIR'] = workspace + args.name + "/wandb/.cache/"
    os.environ['WANDB_CONFIG_DIR'] = workspace + args.name + "/wandb/.config/"
...
def Model(data_loader, model, epoch=None, evaluation=False, testing=False) : 
    ...
    print(' {star} local {local.avg:.3f} interface {interface.avg:.3f} global {_global.avg:.3f} loss {loss.avg:.3f}'.format(
            star=star_label, local=batch_local, interface=batch_interface, _global=batch_global, loss=losses))
